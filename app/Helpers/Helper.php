...
if (!function_exists('match_rfid_api')) {
    function match_rfid_api($r_rfid_no)
    {

        // print_r($r_rfid_no);die;
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://2cqrauto.online/rfdetectordata/api/req/get_trackingdata',
			
			//$current_date = date('d-m-Y');
			//CURLOPT_URL => 'https://2cqrauto.online/rfdetectordata/api/req/get_trackingdata?date=' . $current_date . '',
            ...
            CURLOPT_HTTPHEADER => array(
                'x-api-key: 2LdU8abeXI8NdhGunTxrA8QZBxalKrob13G9bm5R',
                'Authorization: Bearer PNG3zsJ74aDHTzWi703yk5irhzVsJuYeNU4QUfyY'
            ),
        ));
        ...
        // print_r($response1);
        // die;
        // $active_student = SmStudent::where('school_id', Auth::user()->school_id)->where('active_status', 1);

        $temp_rfid = [];

        foreach ($response1 as $key => $apirow) {

            $temp_rfid[$key] = $apirow['rfid_no'];
        }

        // $list = implode(",",$temp_rfid);
        // $attendance = new SmStudentAttendance();

        if (in_array($r_rfid_no, $temp_rfid)) {
...
// start for mart_attendance 

if (!function_exists('mart_attendance')) {
    function mart_attendance()
    {
       $current_date = date('d-m-Y');
        $curl = curl_init();
        ...
        curl_setopt_array($curl, array(
           // CURLOPT_URL => 'https://2cqrauto.online/rfdetectordata/api/req/get_trackingdata',
			...
			CURLOPT_URL => 'https://2cqrauto.online/rfdetectordata/api/req/get_trackingdata?date=03-01-2023',
        ...
        if (isset($response1)) {
            ...
            foreach ($response1 as $record_id => $response) {
        
            //echo '<pre>'; print_r($response['rfid_no']); 
               // $student_record = StudentRecord::where('student_id', '=', $student['id'])->first();
				
				
				$student = SmStudent::where('rfid_no', '=', $response['rfid_no'])->first();
				...
				if(!empty($student))
				   {
					...
					if(!empty($student_record)){
					 ...
					 $att_date = explode(" ", $response['datetime']);
						
 
                    $attendance = SmStudentAttendance::where('student_id', $student_record->student_id)
                        ->where('attendance_date', date('Y-m-d', strtotime($att_date[0])))
                        ->where('class_id', $student_record['class_id'])
                        ->where('section_id', $student_record['section_id'])
                        //->where('student_record_id', $student_record['student_id'])
                        ->where('academic_id', $student_record['academic_id'])
                       ...
                       if (!empty($attendance)) {
                          ...
                          if ($attendance->attendance_type == 'P') {
							 ...
							 DB::table('sm_student_attendances')->delete($attendance->id);

                            $attendance_2 = new SmStudentAttendance();
						    $attendance_2->attendance_type = 'A';
						    $attendance_2->student_id = $student_record->student_id;
                            $attendance_2->student_record_id = $student_record->id;
						    $attendance_2->class_id = $student_record->class_id;
                            $attendance_2->section_id = $student_record->section_id;
                            $attendance_2->notes = 'CRON';
						    $attendance_2->attendance_date = date('Y-m-d', strtotime($att_date[0]));
                            $attendance_2->school_id = $student_record->school_id;
                            $attendance_2->record_id = $student_record->id;
                            $attendance_2->academic_id = getAcademicId();
                            $attendance_2->save();
						
                                     }
                         } else {
						    $attendance_3 = new SmStudentAttendance();
						    $attendance_3->attendance_type = 'P';
						    $attendance_3->student_id = $student_record->student_id;
                            $attendance_3->student_record_id = $student_record->id;
						    $attendance_3->class_id = $student_record->class_id;
                            $attendance_3->section_id = $student_record->section_id;
                            $attendance_3->notes = 'CRON';
						    $attendance_3->attendance_date = date('Y-m-d', strtotime($att_date[0]));
                            $attendance_3->school_id = $student_record->school_id;
                            $attendance_3->record_id = $student_record->id;
                            $attendance_3->academic_id = getAcademicId();
                            $attendance_3->save();
						  
                           }
				}
					// student log start here
					
					  // $datetime = explode(" ", $response['datetime']);
					   // $api_dt = explode(" ", $datetime);
					  //  $api_time = $api_dt[1];
					
					     $datetime = '2023-01-03 17:36:28';
					...
					$rows = SmStudentLog::all();
                    
                    if($rows->isEmpty())
                    {
						//echo "table empty"; die;
                            ...
                            $user_log = new SmStudentLog();
						
                            $user_log->user_id = 207;
						
                            $user_log->role_id = '2';
                          
                            $user_log->school_id = Auth::user()->school_id;
						
                            $user_log->ip_address = $publicIP;
						
                            $user_log->academic_id = getAcademicid() ?? 1;

                            $user_log->user_agent = $agent->browser() . ', ' . $agent->platform();
					
					        $user_log->datetime =  $datetime;
					
					           $user_log->log_type = "Student Present";
					    ...
					    $log = SmStudentLog::where('user_id', 207)
                        ->where('role_id', 2)
                        ->where('academic_id', $student_record['academic_id'])
                        ->where('school_id', $student_record['school_id'])
						->orderBy('id', 'desc')
					...
					if(count($log) >0){
					     $db_datetime =  $log[0]['datetime'];
					     $db_dt = explode(" ", $db_datetime);
					     $db_time = $db_dt[1];
						
						   if($api_time >$db_time && count($rows) >0 ){
							  // echo "api time greater with last row of student"; die;
						  ...
						  // echo "table not empty but api date small from db rows"; die;
						  }
					}
					...
					//student log end here
			}
           }
        }
    }
}
//end for mart_attendance
