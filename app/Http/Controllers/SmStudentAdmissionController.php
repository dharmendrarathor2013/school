...
class SmStudentAdmissionController extends Controller
{
    ...
    public function studentDelete(Request $request)
    {
        try {
            ...
            try {
                ...
                $student_user->save();
                $compact['slug'] = 'student';
                $compact['user_email'] = $student_user->email;
                $compact['name'] = $student_user->full_name;

                @send_sms($student_detail->mobile, 'user_login_permission', $compact);
                ...
                if (count($siblings) == 1) {
                    ...
                    if ($parent) {
                        ...
                        $compact['slug'] = 'parent';
                        $compact['user_email'] = $parent->guardians_email;
                        $compact['name'] = $parent->guardians_name;
                ...
                if ($student_detail->parents && count($siblings) == 1) {
                    $parent_user = User::find($student_detail->parents->user_id);
                    if ($parent_user) {
                        $parent_user->active_status = 0;
                        $parent_user->save();
    ...
    {
        try {
            $studentsArray = ['class_id', 'section_id', 'admission_number', 'roll_no', 'first_name', 'last_name', 'date_of_birth', 'religion', 'gender', 'caste', 'mobile', 'email', 'admission_date', 'blood_group', 'height', 'weight', 'father_name', 'father_phone', 'father_occupation', 'mother_name', 'mother_phone', 'mother_occupation', 'guardian_name', 'guardian_relation', 'guardian_email', 'guardian_phone', 'guardian_occupation', 'guardian_address', 'current_address', 'permanent_address', 'bank_account_no', 'bank_name', 'national_identification_no', 'local_identification_no', 'previous_school_details', 'note'];
    ...
    public function disabledStudentDelete(Request $request)
    {
        try {
            ...
            try {
                ...
                $parent_user = @$student_detail->parents->user_id;
                $siblings = SmStudent::withOutGlobalScope(StatusAcademicSchoolScope::class)->where('parent_id', $student_detail->parent_id)->wherenotNull('parent_id')->where('school_id', Auth::user()->school_id)->get();
                DB::beginTransaction();
                if ($student_detail->student_photo != "") {
                    if (file_exists($student_detail->student_photo)) {
                        unlink($student_detail->student_photo);
                ...
                if (count($siblings) == 1 && $parent_user) {
