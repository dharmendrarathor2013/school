...
class SmStaffController extends Controller
{
    ...
    public function staffStore(staffRequest $request)
    {
        ...
        try {
         ...
         $trole_name = DB::table('infix_roles')->select('name')->where('id', $request['role_id'])->first();
         $role_name = $trole_name->name;
            DB::beginTransaction();
            try {
            ...
            $user_log->log_type = "created ".$role_name;
    ...
    public function importStaff()
    {
        ...
        try {
            ...
            //    echo "<pre>"; print_r($max_staff_no); die;
            ...
            $custom_fields = SmCustomField::where('form_name', 'staff_registration')->get();
            $is_required = SmStaffRegistrationField::where('school_id', auth()->user()->school_id)->where('is_required', 1)->pluck('field_name')->toArray();
            return view('backEnd.humanResource.importStaff', compact('roles', 'departments', 'designations', 'marital_ststus', 'max_staff_no', 'genders', 'custom_fields', 'is_required'));
        } catch (\Exception $e) {
            return $e->getMessage();
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        ...
        //return view('backEnd.humanResource.importStaff');
    }



    //download sample file
    public function downloadStaffFile()
    {
        try {
    ...
    // staff bulk store
   public function staffBulkStore(Request $request)
    {
        $request->validate(
            [
                'file' => 'required'
        ...
        $staffCount = 0;
        $staffdata = SmStaff::all();
        $staffCount =  $staffdata->count();
        // echo $staffCount; exit;

        //   $arr =   [
        //         "status" => 1
        //     ];

        //   $info =   $this->users->model($arr); 
        ...
        if ($file_type <> 'csv' && $file_type <> 'xlsx' && $file_type <> 'xls') {
            ...
            StaffBulkTemporary::where('user_id', Auth::user()->id)->delete();
            $path = $request->file('file');
            Excel::import(new StaffsImport, $request->file('file'), 's3', \Maatwebsite\Excel\Excel::XLSX);

             
            $data1 = StaffBulkTemporary::where('user_id', Auth::user()->id)->get();
            ...
            foreach ($data1 as $key => $value) {
                ...
                $users = StaffBulkTemporary::all();
                $usersUnique = $users->unique(['email']);
                $userDuplicates = $users->diff($usersUnique);
                if ($userDuplicates->toArray()) {
                    StaffBulkTemporary::where('user_id', Auth::user()->id)->delete();
                ...
                $temp_role_id = DB::table('infix_roles')->select('id')->where('name', $value['role_name'])->first();
                $temp_department_id = DB::table('sm_human_departments')->select('id')->where('name', $value['department_name'])->first();
                $temp_designation_id = DB::table('sm_designations')->select('id')->where('title', $value['designation_name'])->first();
                ...
                if ($temp_role_id == "") {
                    ...
                    $value['role_id'] = $temp_role_id->id;
                }


                if ($temp_department_id == "") {
                    Toastr::error('Department does not exist');
                    return redirect()->back();
                    exit;
                } else {
                    $value['department_id'] = $temp_department_id->id;
                }


                if ($temp_designation_id == "") {
                    Toastr::error('Designation does not exist');
                    return redirect()->back();
                    exit;
                } else {
                    $value['designation_id'] = $temp_designation_id->id;
                }
                ...
                if ($value['role_name'] == '') {
                    Toastr::error('Role Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['department_name'] == '') {
                    Toastr::error('Department Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['designation_name'] == '') {
                    Toastr::error('Designation Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['gender_name'] == '') {
                    Toastr::error('Gender Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['current_address'] == '') {
                    Toastr::error('Address Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['basic_salary'] == '') {
                    Toastr::error('Salary Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['email'] == '') {
                    Toastr::error('Email Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['first_name'] == '') {
                    Toastr::error('First name Can not be null');
                    return redirect()->back();
                    exit;
                }
                if ($value['last_name'] == '') {
                    Toastr::error('Last name Can not be null');
                    return redirect()->back();
                    exit;
                }


                if ($value['role_name'] == '' && $value['department_name'] == '' && $value['designation_name'] == '' && $value['gender_name'] == '' && $value['current_address'] == '' && $value['basic_salary'] == '' && $value['email'] == '' && $value['first_name'] == '' && $value['last_name'] == '' && $value['mobile'] == '') {
                } else {
                    $data[$i] = $value;
                    $data[$i]['id'] = $j;
                    $i++;
           ...
           // $staff
            if (!empty($data)) {
                try {
                    foreach ($data as $key => $value) {
						//echo $key; die;
                $role_names[$count_r] = $value->role_name;
						//$count_r = $count_r +1;
                        $staffCount = $staffCount + 1;

                        $user = new User();
                        $user->role_id = $value->role_id;
                        $user->username = $value->mobile ? $value->mobile : $value->email;
                        ...
                        $basic_salary = !empty($value->basic_salary) ? $value->basic_salary : 0;

                        $staff = new SmStaff();
                        $staff->staff_no = $staffCount;
                        $staff->role_id = $value->role_id;
                        $staff->department_id = $value->department_id;
                        $staff->designation_id = $value->designation_id;
                        ...
                        $staff->first_name = $value->first_name;
                        $staff->last_name = $value->last_name;
                        $staff->full_name = $value->first_name . ' ' . $value->last_name;
                        $staff->fathers_name = $value->fathers_name;
                        $staff->mothers_name = $value->mothers_name;
                        $staff->email = $value->email;
                        $staff->school_id = Auth::user()->school_id;
                        // $staff->gender_id = $value->gender_id;


                          if($value->gender_name=='Male'){
                            $staff->gender_id = 1;
                          }
                          if($value->gender_name=='Female'){
                            $staff->gender_id = 2;
                          }
                          if($value->gender_name=='Others'){
                            $staff->gender_id = 3;
                          }


                        $staff->marital_status = $value->marital_status;
                        $staff->date_of_birth = date('Y-m-d', strtotime($value->date_of_birth));
                        $staff->date_of_joining = date('Y-m-d', strtotime($value->date_of_joining));
                        $staff->mobile = $value->mobile ?? null;
                        $staff->emergency_mobile = $value->emergency_mobile;
                        $staff->current_address = $value->current_address;
                        $staff->permanent_address = $value->permanent_address;
                        $staff->qualification = $value->qualification;
                        $staff->experience = $value->experience;
                        $staff->epf_no = $value->epf_no;
                        $staff->basic_salary = $basic_salary;
                        $staff->contract_type = $value->contract_type;
                        $staff->location = $value->location;
                        $staff->bank_account_name = $value->bank_account_name;
                        $staff->bank_account_no = $value->bank_account_no;
                        $staff->bank_name = $value->bank_name;
                        $staff->bank_brach = $value->bank_brach;
                        $staff->facebook_url = $value->facebook_url;
                        $staff->twiteer_url = $value->twiteer_url;
                        $staff->linkedin_url = $value->linkedin_url;
                        $staff->instragram_url = $value->instragram_url;
                        $staff->user_id = $user->id;
                        $staff->driving_license = $value->driving_license;
            ...
            $user_log->user_id = Auth::user()->id;
            $user_log->role_id = Auth::user()->role_id;
            $user_log->log_type = "created ".implode(" ,",$role_names);
            $user_log->school_id = Auth::user()->school_id;
            $user_log->ip_address = $request->ip();
            $user_log->academic_id = getAcademicid() ?? 1;
